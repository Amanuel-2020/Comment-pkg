    ...
    <div class="tab__box">
        <div class="tab__items">
            ...
            <a class="tab__item {{request('status') == \Abd\Comment\Models\Comment::STATUS_NEW  ? 'is-active' : ""}}"
               href="{{route('comments.index')}}?status={{\Abd\Comment\Models\Comment::STATUS_NEW}}">Unapproved
                Comments</a>
            <a class="tab__item {{request('status') == \Abd\Comment\Models\Comment::STATUS_REJECTED  ? 'is-active' : ""}}"
               href="{{route('comments.index')}}?status={{\Abd\Comment\Models\Comment::STATUS_REJECTED}}">Rejected
                Comments</a>
            <a class="tab__item {{request('status') == \Abd\Comment\Models\Comment::STATUS_APPROVED  ? 'is-active' : ""}}"
               href="{{route('comments.index')}}?status={{\Abd\Comment\Models\Comment::STATUS_APPROVED}}">Approved
                Comments</a>
        </div>
    </div>
            ...
            <form action="">
                <div class="t-header-searchbox font-size-13">
                    ...
                    <div class="t-header-search-content">
                        ...
                        <input type="text" class="text margin-bottom-20" name="name" placeholder="First and last name">
